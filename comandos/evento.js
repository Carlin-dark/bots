...
export async function handleEventCommand(body, chatId, sock) {
  ...
  if (args.length < 3) {
    return sock.sendMessage(chatId, { text: "⚠️ *Uso correto:* /evento DD/MM HH:MM Descrição do evento" });
  }
  ...
  if (!dateTime || description.length === 0) {
    return sock.sendMessage(chatId, { text: "❌ Formato inválido. Use: /evento DD/MM HH:MM Descrição" });
  }
